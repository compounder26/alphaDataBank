...
def calculate_alpha_correlation_fast(
    np.ndarray[double, ndim=1] alpha_pnl, 
    np.ndarray[double, ndim=1] other_pnl,
    ...
    cdef np.ndarray[double, ndim=1] alpha_daily = np.zeros(n, dtype=np.float64)
    cdef np.ndarray[double, ndim=1] other_daily = np.zeros(n, dtype=np.float64)
    cdef np.ndarray[double, ndim=1] alpha_port_val = np.zeros(n, dtype=np.float64)
    cdef np.ndarray[double, ndim=1] other_port_val = np.zeros(n, dtype=np.float64)
    cdef np.ndarray[double, ndim=1] alpha_returns = np.zeros(n, dtype=np.float64)
    cdef np.ndarray[double, ndim=1] other_returns = np.zeros(n, dtype=np.float64)
    ...
    cdef np.ndarray[np.uint8_t, cast=True] valid_mask = np.logical_and(
        np.logical_and(~np.isnan(alpha_returns), ~np.isnan(other_returns)),
        np.logical_and(~np.isinf(alpha_returns), ~np.isinf(other_returns))
    )
    
    cdef np.ndarray[double, ndim=1] alpha_returns_clean = alpha_returns[valid_mask]
    cdef np.ndarray[double, ndim=1] other_returns_clean = other_returns[valid_mask]
